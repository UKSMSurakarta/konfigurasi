...
import { getRekapSekolahApi, getAdminDashboardApi } from "../../api/admin";
...
export default function SuperAdminLaporanDetail() {
    ...
    useEffect(() => {
        setLoading(true);
        Promise.all([
            getRekapSekolahApi({ opd_id: opdId }),
            getAdminDashboardApi({ opd_id: opdId })
        ])
            .then(([rekapRes, dashRes]) => {
                ...
                // Get OPD name from the first school if available
                if (schoolsList && schoolsList.length > 0 && schoolsList[0].opd) {
                    setOpdName(schoolsList[0].opd);
                }
            })
            .catch(console.error)
                ...
                <div style={{ flex: 1, minWidth: 0, display: "flex", alignItems: "flex-start", gap: "12px" }}>
                    ...
                    <div>
                        ...
                        <p className="text-muted" style={{ fontSize: "14px", lineHeight: 1.6 }}>
                            Menampilkan data assessment dan progres untuk Admin OPD: <strong>{opdName || `OPD ID ${opdId}`}</strong>
                        </p>
                    </div>
                </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "18px" }}>
                ...
                <div className="card glass-panel" style={{ padding: "24px", borderRadius: "24px" }}>
                    <div className="flex items-center gap-2" style={{ marginBottom: "22px" }}>
                        ...
                        <h3 style={{ fontSize: "18px", fontWeight: 700 }}>Analisis Strata (OPD Ini)</h3>
                    </div>
                ...
                <div className="card glass-panel" style={{ padding: "28px", borderRadius: "28px", minWidth: 0 }}>
                    ...
                    <p className="text-muted" style={{ fontSize: "13px", marginBottom: "24px" }}>Rekap status assessment sekolah binaan OPD ini</p>
            ...
            <div className="card glass-panel" style={{ padding: "24px", borderRadius: "24px" }}>
                <div className="flex items-center justify-between" style={{ flexWrap: "wrap", gap: "14px", marginBottom: "20px" }}>
                    <div className="flex items-center gap-2">
                        ...
                        <h3 style={{ fontSize: "18px", fontWeight: 700 }}>Rekap Sekolah Binaan OPD</h3>
                    </div>
                </div>
                <div style={{ overflowX: "auto", borderRadius: "18px", border: "1px solid var(--border)" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "720px" }}>
                        ...
                        <tbody>
                            {schools.length === 0 && (
                                <tr><td colSpan={5} style={{ textAlign: "center", padding: "32px", color: "var(--text-muted)" }}>Belum ada data sekolah pada OPD ini.</td></tr>
                            )}
                            {schools.map(s => (
                                <tr key={s.id}>
