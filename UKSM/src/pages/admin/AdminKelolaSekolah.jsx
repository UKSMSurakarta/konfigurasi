...
export default function AdminKelolaSekolah() {
  ...
  function openEdit(s) {
    setEditTarget(s.id);
    const u = (s.users && s.users.length > 0) ? s.users[0] : null;
    setForm({
      nama: s.nama, npsn: s.npsn, jenjang: s.jenjang,
      kepala_sekolah: s.kepala_sekolah || "", alamat: s.alamat || "",
      user_email: u ? u.email : "", user_password: ""
    });
    setModalOpen(true);
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (editTarget) {
        ...
        const res = await createAdminSekolahApi(form);
        setSuccessData(res.data?.data || res.data || res);
        setCurrentPage(1);
        closeModal();
        fetchSekolahs();
      }
  ...
  return (<div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
    <div style={{ background: "var(--card-bg)", borderRadius: "24px", padding: "28px 32px", maxWidth: "520px", width: "100%", maxHeight: "90vh", overflowY: "auto" }}>
      ...
      <form onSubmit={onSubmit}>
        ...
        <div style={{ marginTop: "20px", marginBottom: "12px", borderTop: "1px solid var(--border)", paddingTop: "16px" }}>
          <h3 style={{ fontSize: "14px", fontWeight: 700, marginBottom: "16px", color: "var(--primary)" }}>
            Kredensial Akun (Opsional)
          </h3>
          <FormField label="Email Akun Sekolah">
            <input type="email" value={form.user_email} onChange={(e) => setForm({ ...form, user_email: e.target.value })} placeholder={editTarget ? "Email akun..." : "Kosongkan untuk generate otomatis"} style={inputStyle} />
          </FormField>
          <FormField label={editTarget ? "Setel Password Baru" : "Password Akun"}>
            <input type="text" value={form.user_password} onChange={(e) => setForm({ ...form, user_password: e.target.value })} placeholder={editTarget ? "Kosongkan jika tidak ingin diubah" : "Kosongkan untuk generate otomatis"} style={inputStyle} />
          </FormField>
        </div>
...
function SuccessModal({ data, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1050, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
      <div className="card glass-panel" style={{ borderRadius: "24px", padding: "32px", maxWidth: "460px", width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.25)" }}>
        <div style={{ width: 56, height: 56, borderRadius: "50%", background: "#DCFCE7", display: "flex", alignItems: "center", justifyContent: "center", color: "#16A34A", marginBottom: "20px" }}>
          <CheckCircle2 size={28} />
        </div>
        <h2 style={{ fontSize: "22px", fontWeight: 700, marginBottom: "8px" }}>Sekolah Berhasil Ditambahkan</h2>
        <p style={{ fontSize: "14px", color: "var(--text-muted)", lineHeight: 1.6, marginBottom: "20px" }}>
          Akun untuk <strong>{sekolah?.nama || 'Sekolah Baru'}</strong> telah berhasil didaftarkan.
        </p>

        <div style={{ background: "var(--bg-light)", padding: "16px", borderRadius: "12px", marginBottom: "24px", border: "1px solid var(--border)" }}>
          <div style={{ marginBottom: "12px" }}>
            <div style={{ fontSize: "12px", color: "var(--text-muted)", fontWeight: 600 }}>Email Login</div>
            <div style={{ fontSize: "15px", fontWeight: 700, color: "var(--primary)", userSelect: "all" }}>{user?.email || '-'}</div>
          </div>
          <div>
            <div style={{ fontSize: "12px", color: "var(--text-muted)", fontWeight: 600 }}>Password</div>
            <div style={{ fontSize: "15px", fontWeight: 700, color: "var(--text-main)", userSelect: "all" }}>{user?.password || '-'}</div>
          </div>
        </div>

        <div style={{ padding: "12px", background: "#FEF3C7", borderRadius: "10px", color: "#B45309", fontSize: "12px", fontWeight: 600, display: "flex", alignItems: "flex-start", gap: "8px", marginBottom: "24px" }}>
          <AlertCircle size={16} style={{ flexShrink: 0, marginTop: 1 }} />
          <span>Tolong salin dan simpan informasi akun sekolah ini. Password hanya ditampilkan sekali ini saja, dan bisa direset jika lupa.</span>
        </div>

        <button onClick={onClose} style={{ width: "100%", height: "46px", borderRadius: "12px", border: "none", background: "linear-gradient(135deg,#0F6E56,#0D5C48)", color: "white", fontWeight: 700, cursor: "pointer" }}>
          Saya Mengerti
        </button>
      </div>
    </div>
