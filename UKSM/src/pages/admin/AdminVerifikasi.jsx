...
const PREDIKAT_LIST = [
  ...
  },
  {
    key: "standar",
    label: "Standar",
    deskripsi: "Memenuhi standar dasar",
    color: "#3B82F6",
    bg: "#DBEAFE",
  },
  {
...
export default function AdminVerifikasi() {
  ...
  const filtered = schools.filter((s) => {
    ...
    const matchSearch =
      (s.nama || s.name || "").toLowerCase().includes(q) ||
  ...
  const menunggu = schools.filter(
    (s) => s.status === "Menunggu Verifikasi",
  ).length;
  const belum = schools.filter(
    (s) => !["Terverifikasi", "Menunggu Verifikasi"].includes(s.status),
  ).length;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ padding: "18px", borderRadius: "22px" }}
      >
        <div
          style={{
            display: "grid",
        ...
        >
          <div style={{ position: "relative" }}>
            ...
            <input
              type="text"
              placeholder="Cari sekolah / wilayah..."
              value={search}
      ...
      >
        {filtered.map((school) => {
          ...
          const canVerify =
            (school.status === "Menunggu Verifikasi" || progress > 0) &&
            !isVerified;
          ...
          return (
            ...
            >
              ...
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div className="text-muted" style={{ fontSize: "13px" }}>
                    {school.jenjang || "–"} ·{" "}
                    {school.wilayah || school.opd?.nama || "–"}
                  </div>
                </div>
              ...
              >
                <button
                  onClick={() => navigate(`${basePath}/${school.id}`)}
                  style={{ ...detailBtn }}
                >
                  <Eye size={15} /> Detail
                </button>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div style={{ fontSize: "13px", color: "var(--text-muted)" }}>
                    {modalSchool.nama || modalSchool.name}
                  </div>
                </div>
              </div>
              <button
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              ...
              <button
                onClick={() =>
                  handleVerify(
                    modalSchool.id,
                    modalSchool.active_level_id || modalSchool.level_id,
                    "ditolak",
                  )
                }
                disabled={confirming}
